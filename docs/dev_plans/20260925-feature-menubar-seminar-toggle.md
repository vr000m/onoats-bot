...
**Assigned to**: Claude
**Priority**: Low
**Branch**: feature/menubar-seminar-toggle
**Created**: 2026-09-25
**Review Gates**: none

## Objective

Let the user mark the next recording as a seminar from the menu bar. Other categories are classified downstream, so only `seminar` needs a manual override.

## Requirements

- A "Seminar (next recording)" toggle in the menu; on → Start runs `onoats bot --category seminar` (the existing `--category` flag; no Python change).
...
- `seminar` must be in `[categories] set`; otherwise `validate_category` rejects it and Start fails with the CLI's error. Not pre-checked in Swift because `ConfigStore` reads scalars only, not TOML arrays.
...
- [ ] Manual: `make -C native install`, add `seminar` to `[categories] set`, toggle on, Start, confirm the queue file's `session_meta` line says `seminar`; confirm the toggle is off after Stop; confirm Start fails visibly when `seminar` is not in the set
